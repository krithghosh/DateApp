...
class ExpandableItemIndicatorImplAnim extends ExpandableItemIndicator.Impl {
    ...
    public void setExpandedState(boolean isExpanded, boolean animate) {
        /*if (animate) {
            if (isExpanded) {
                mImageView.setImageResource(R.drawable.ic_expand_more_to_expand_less);
            } else {
                mImageView.setImageResource(R.drawable.ic_expand_less_to_expand_more);
            }
            ((Animatable) mImageView.getDrawable()).start();
        } else {
            @DrawableRes int resId = isExpanded ? R.drawable.ic_less : R.drawable.ic_expand;
