...
import com.example.krith.dates.TransferObjects.CommonSessions;
...
public class ExpandableDataProvider extends AbstractExpandableDataProvider {
    ...
    public ExpandableDataProvider(Sessions sessions) {
        try {
            final List<String> groupItems = new ArrayList<String>();
            groupItems.add("Morning");
            groupItems.add("Afternoon");
            groupItems.add("Evening");
            final List<List<CommonSessions>> childItems = new ArrayList<List<CommonSessions>>();
            childItems.add(sessions.getMorning());
            childItems.add(sessions.getAfternoon());
            childItems.add(sessions.getEvening());
            ...
            for (int i = 0; i < groupItems.size(); i++) {
                ...
                for (int j = 0; j < childItems.get(i).size(); j++) {
                    if (!childItems.get(i).get(j).getBooked() && !childItems.get(i).get(j).getExpired()) {
                        count++;
                    }
                }

                final long groupId = i;
                final ConcreteGroupData group = new ConcreteGroupData(groupId, groupItems.get(i), count);
                ...
                for (int j = 0; j < childItems.get(i).size(); j++) {
                    ...
                    try {
                        ...
                        Date startDate = dateFormat.parse(childItems.get(i).get(j).getStartTime());
                        Date endDate = dateFormat.parse(childItems.get(i).get(j).getEndTime());
                        String childText = newFormat.format(startDate) + " - " + newFormat.format(endDate);
                        if (!childItems.get(i).get(j).getBooked() && !childItems.get(i).get(j).getExpired()) {
                            isAvailable = true;
                        } else {
                            isAvailable = false;
                        }
                        String _childText = childText.replace(".", "").toUpperCase();
                        children.add(new ConcreteChildData(childId, _childText, isAvailable));
    ...
    public static final class ConcreteGroupData extends GroupData {
        ...
        ConcreteGroupData(long id, String text, int count) {
            mId = id;
            mText = text;
            mNextChildId = 0;
            mCount = count;
    ...
    public static final class ConcreteChildData extends ChildData {
        ...
        ConcreteChildData(long id, String text, boolean isAvailable) {
            mId = id;
            mText = text;
